...
func _on_message_finished() -> void:
	var text: FloatingText = Helpers.create_floating_text(LevelLoader.level_ui_nodes, ("+" if rocks_amount >= 0 else "") + str(int(rocks_amount)) + " R", global_position + Vector2(0, -24), text_color, -10)
